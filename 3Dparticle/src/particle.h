...
public:
    ...
    Particle(float m, float r, int x, int y, float vx, float vy, ofColor c = ofColor(200, 200, 200))
    {
        color = c;
        mass = m;
        this->r = r;
        ...
        vel.x = vx;
        vel.y = vy;
        acc = ofVec2f(0.0f, 0.0f);
    }

    ofVec2f calcA(Particle &otherDisk, float G = 0.1, float dt = 1.0f)
    {
    ...
    void calcVelocity(float dt = 0.1f)
    {
        ...
        if (vel.x > 1000.0f)
            vel.x = 1000.0f;
        else if (vel.x < -1000.0f)
            vel.x = -1000.0f;

        if (vel.y > 1000.0f)
            vel.y = 1000.0f;
        else if (vel.y < -1000.0f)
            vel.y = -1000.0f;
    }
